	...
	private void collectGarbage() {
		...
		registry.entrySet().stream().forEach(e -> {
			...
			if(s.isRefreshedSinceCompletion() || isEvictionTimeReached(s)){
				...
				registry.remove(id);
				logger.info("Evicted stream " + id + " - Duration: "+s.getDurationMs()+" ms. Reason: consumed="+s.isRefreshedSinceCompletion() + ", evictionTimeReached=" + isEvictionTimeReached(s) + " (eviction timeout is "+ this.defaultTimeToEviction +" ms).");
			}
		});
	}
