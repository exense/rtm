            ...
            <% for (i = 0; i < tlength; i++) { %>
              <% curMetric = checkedAggTableMetrics[i]; %>
                <%if($.inArray(curMetric, dateMetric) >= 0){%>
                  <%var curDate = new Date(itemZ.numericalAttributes[curMetric]);%>
                  <%var printableDate = getPrintableDate(curDate)%>
                    <td><%=printableDate%>(<%=itemZ.numericalAttributes[curMetric]%>)</td>
                <%}else{%>
                    <td><%= itemZ.numericalAttributes[curMetric] %></td>
                <%}%>
              <% } %>
              </tr>
              <% }); %>
